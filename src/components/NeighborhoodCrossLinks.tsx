        ...
        <div className="grid md:grid-cols-2 gap-6">
          {otherPlaces.map((place) => (
            <Link
              key={place.key}
              to={place.path}
              className="group relative h-48 rounded-xl overflow-hidden shadow-md hover:shadow-lg transition-shadow"
            >
              <img src={place.img} alt={t(place.titleKey)} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
              <div className="absolute inset-0 bg-black/40 group-hover:bg-black/50 transition-colors" />
              <div className="relative h-full flex flex-col justify-end p-6 text-white">
                <p className="text-xs uppercase tracking-widest opacity-80 mb-1">{t(place.distanceKey)}</p>
                <div className="flex items-center gap-2">
                  <h3 className="text-xl font-serif">{t(place.titleKey)}</h3>
