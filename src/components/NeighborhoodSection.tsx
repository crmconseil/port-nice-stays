    ...
    <section className="section-padding bg-background">
      <div className="container-narrow">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }} className="text-center mb-12">
          <p className="text-primary uppercase tracking-[0.2em] text-sm mb-3 font-sans">{t('neighborhood.subtitle')}</p>
          <h2 className="text-4xl md:text-5xl font-serif text-foreground mb-4">{t('neighborhood.title')}</h2>
          <p className="text-muted-foreground max-w-2xl mx-auto">{t('neighborhood.description')}</p>
        </motion.div>
          ...
          {areas.map((area, index) => {
            ...
            return (
              <motion.div key={area.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.1 }} viewport={{ once: true }}>
                {area.link ? (
                  <Link to={area.link} className="block bg-card rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow border border-border">{Content}</Link>
                ) : (
                  <div className="bg-card rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow border border-border">{Content}</div>
                )}
              </motion.div>
            );
          })}
